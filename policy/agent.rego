...
# ★ 指定ステップ i に紐づく WARN の理由を配列で返す
reasons_for_step(i) = rs {
  warn_defined
  rs := [r |
    w := data.agent.policy.warn[_]
    related_to_step(i, w)
    r := {
      "kind":     object.get(w, "kind", "unspecified"),
      "severity": lower(object.get(w, "severity", "low")),
      "message":  object.get(w, "message", "")
    }
  ]
} else = [] {
  not warn_defined
